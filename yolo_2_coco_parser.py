...
def yolo2coco(image_dir_path, label_dir_path, save_file_name , is_normalized):
    ...
    for image_name in image_list :
        ...
        label_path = os.path.join(label_dir_path, os.path.splitext(image_name)[0] + '.txt')

        label = open(label_path,'r')
        #label = open(label_dir_path+image_name[0:-4] + '.txt','r')
        if not os.path.isfile(label_path): # debug code
            print('there is no label match with :', label_path)
            return
        while True:
            line = label.readline()
            if not line:
...
if __name__ == '__main__':
    image_dir_path = '/home/etriobj/jupyter/etri/data/0506_V0001/image'
    label_dir_path = '/home/etriobj/jupyter/etri/data/0506_V0001/label'
    save_file_name = '/home/etriobj/jupyter/etri/data/instances_train2017.json'
    is_normalized = True
